...
const CREATED_BY = 'seed-new-suites-30';

// Deliberately different pass/fail/skip shapes per case role, so both the
// per-run pass rate and the per-case flakiness classification vary — mirrors
// a real CI history where different tests fail for different reasons.

function stablePass(n) {
  // Passes every run except the most recent one — "just started failing."
  return Array.from({ length: n }, (_, i) => (i === n - 1 ? 'failed' : 'passed'));
}

function stableFail(n) {
  // Fails every run except the most recent one — "just got fixed."
  return Array.from({ length: n }, (_, i) => (i === n - 1 ? 'passed' : 'failed'));
}

function flakyFrequent(n) {
  // Flips every run — clearly flaky.
  return Array.from({ length: n }, (_, i) => (i % 2 === 0 ? 'passed' : 'failed'));
}

function flakyModerate(n) {
  // Mostly passing with two isolated failure blips.
...
function mostlySkippedStablePass(n) {
  // Often skipped (environment issues), but consistently passes when it runs.
...
function seedNewSuiteRuns() {
  ...
  console.log(`Seeded ${totalSeeded} test runs across the new suites.`);
}

module.exports = seedNewSuiteRuns;
